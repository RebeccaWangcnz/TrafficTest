...
public class MenuOverlay : MonoBehaviour
	...
	void OnCharts (bool value)
		...
		if (vehicle != null)
			{
			VPPerformanceDisplay telemetryChart = vehicle.GetComponentInChildren<VPPerformanceDisplay>();

			if (telemetryChart != null)
				{
				telemetryChart.visible = value;

				if (chartsToolbar != null)
					{
					chartsToolbar.telemetryChart = telemetryChart;
					SetEnabled(chartsToolbar, value);
					}
				}
			else
				{
		...
		// DEBUG: Developing the chart within the test scene
		// SetEnabled(chartsToolbar, value);
		}
