...
public class Dashboard : MonoBehaviour
	...
	void Update ()
		...
		float speedMs = MathUtility.FastAbs(m_speedMs.GetInterpolated(frameRatio));
		float engineRpm = MathUtility.FastAbs(m_engineRpm.GetInterpolated(frameRatio));
		...
		if (vehicle.time > m_lastVehicleTime)
			...
			if (speedMphLabel != null)
				speedMphLabel.text = $"{(speedMs * 2.237f):0}\nmph";
