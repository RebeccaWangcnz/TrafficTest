...
//
// These aids must be enabled in the vehicle inspector to be properly activated/deactivated here.
// This script has no effect on disabled aids.
...
public class AidsPanel : MonoBehaviour
	...
	void Start ()
		...
		SetSteeringAidEnabled(steeringEnabled);
		}
	...
	void Update ()
		...
		// Inhibit steering assist when using device input

		#if !VPP_ESSENTIAL
		if (m_deviceInput != null)
			steeringAidAvailable = !m_deviceInput.enabled;
		#endif

		// Read the settings from the vehicle
