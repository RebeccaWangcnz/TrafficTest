                ...
                for (int i = 0; i < carCount; i++) // operate on results
                {
                    if (isActiveNL[i] && canProcessNL[i])
                    {
                        ...
                        else if (overrideInputNL[i])//如果触发重写
                        {
                            ...
                            if (!frontHitNL[i] & speedNL[i] - targetSpeedNL[i]>20f)//超速
                            {
                                rigidbodyList[i].drag = minDragNL[i];
                                brakeTorqueNL[i] = 0.2f*brakePowerNL[i];
                            }
                            else if(!frontHitNL[i] & speedNL[i] - targetSpeedNL[i] > 20f)
                            {
                        ...
                        if (isBrakingNL[i])//连续刹车时间大于一定时间才亮刹车灯
                        {
                            ...
                            if (brakeTimeNL[i] > 0.1f)
                            {
                                if (brakeTorqueNL[i] > 0 || (speedNL[i]>60f &&speedNL[i] - targetSpeedNL[i]>20f)|| (speedNL[i] < 60f && speedNL[i] - targetSpeedNL[i] > 10f))//限定刹车灯亮起条件：踩刹车/长减速
                                {
